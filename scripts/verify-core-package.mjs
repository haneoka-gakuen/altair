...
const unexpectedExports = Object.keys(packageJson.exports).filter(
  (entry) => !allowedExports.has(entry),
);
if (unexpectedExports.length) {
  throw new Error(
    `Altair core exposes non-kernel entry points: ${unexpectedExports.join(", ")}`,
  );
}
...
if (unexpectedDependencies.length) {
  throw new Error(
    `Altair core has non-kernel runtime dependencies: ${unexpectedDependencies.join(", ")}`,
  );
}
...
if (forbiddenPaths.length) {
  throw new Error(
    `Altair core package contains feature artifacts:\n${forbiddenPaths.join("\n")}`,
  );
}
...
for (const path of paths.filter((path) => path.endsWith(".js"))) {
  ...
  const matches = forbiddenRuntimeSymbols.filter((symbol) =>
    source.includes(symbol),
  );
  if (matches.length) {
    throw new Error(
      `${path} contains feature implementation symbols: ${matches.join(", ")}`,
    );
  }
  if (
    /\b(?:fetch|XMLHttpRequest|WebSocket|EventSource)\s*\(/u.test(source) ||
    /\bglobalThis\.fetch\b/u.test(source)
  ) {
...
const rootModule = await import(
  `${pathToFileURL(resolve(root, "dist/index.js")).href}?boundary-audit`
);
const leakedExports = forbiddenRuntimeSymbols.filter(
  (symbol) => symbol in rootModule,
);
if (leakedExports.length) {
  throw new Error(
    `Altair root exports feature implementations: ${leakedExports.join(", ")}`,
  );
}
...
const unexpectedRuntimeExports = Object.keys(rootModule).filter(
  (name) => !allowedRuntimeExports.has(name),
);
if (unexpectedRuntimeExports.length) {
  throw new Error(
    `Altair root has non-kernel runtime exports: ${unexpectedRuntimeExports.join(", ")}`,
  );
}

console.log(
  `Altair core package boundary verified: ${report.entryCount} files, ${report.unpackedSize} unpacked bytes`,
);
