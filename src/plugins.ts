import { AltairDocumentRegistry, type AltairAuthoredWorkspace } from "./documents.js";
import { satisfies, valid, validRange } from "semver";
import type { AltairAdaptationRequest, AltairAdaptationResult, AltairAiProvider } from "./ai-protocol.js";
...
import {
  ...
  type AltairEditorWorkspace,
  type AltairEditorDocument,
  type AltairDocumentPreview,
...
import type { ResourceBrowserDirectory, ResourceBrowserProvider } from "./resource-browser.js";
...
export interface AltairDiagnosticRule {
  readonly id: string;
  evaluate(project: StoryProject): readonly StoryDiagnostic[] | Promise<readonly StoryDiagnostic[]>;
}
...
export type AltairPluginManifest = AltairPluginManifestV1 | AltairPluginManifestV2;
...
export interface AltairPluginV2 {
  ...
  setup(
    ...
    | (() => void | Promise<void>)
    | Promise<void | AltairPluginActivation | (() => void | Promise<void>)>;
}
...
const CAPABILITY_BY_CONTRIBUTION: Readonly<Record<keyof AltairContributionMap, AltairPluginCapability>> = Object.freeze(
  {
...
export class AltairPluginHost {
  ...
  private readonly legacyFormats = new Map<string, RegisteredLegacy<AltairFormatCodec>>();
  private readonly legacyAiProviders = new Map<string, RegisteredLegacy<AltairAiProvider>>();
  private readonly legacyDiagnosticRules = new Map<string, RegisteredLegacy<AltairDiagnosticRule>>();
  ...
  list(): readonly AltairPluginManifest[] {
    return Object.freeze([...this.installed.values()].map(({ manifest }) => manifest));
  }

  compileDocuments(workspace: AltairAuthoredWorkspace): StoryProject {
    const registry = new AltairDocumentRegistry();
    for (const selection of this.contributionSelections("command")) {
      const definition = selection.contribution.document;
      if (!definition) continue;
      if (definition.type.plugin !== selection.owner)
        throw new Error(`Command provider cannot claim another plugin namespace: ${definition.type.plugin}`);
      registry.register(definition);
    }
    return registry.compile(workspace);
  }

  contributions<K extends keyof AltairContributionMap>(kind: K): readonly AltairContributionMap[K][] {
  ...
  contributionSelections<K extends keyof AltairContributionMap>(
    ...
    const records = [...registry.values()].filter(({ owner }) => this.installed.has(owner));
    ...
    return Object.freeze(
      records.map((entry) => {
        ...
        return Object.freeze({
          ...
          ...(entry.singletonPort === undefined ? {} : { singletonPort: entry.singletonPort }),
          active: winner === entry,
          ...(winner && winner !== entry ? { suppressedBy: qualifiedContributionId(winner) } : {}),
        });
      }),
    );
  }
  ...
  service<T>(key: AltairServiceKey<T>): T | undefined {
    ...
    return service && this.installed.has(service.owner) ? (service.value as T) : undefined;
  }

  commandSchema(selector: number | string): AltairCommandSchemaContribution | undefined {
  ...
  async createCommand(
    ...
    if (!selection.contribution.create) {
      throw new Error(`Altair command schema ${selection.owner}:${selection.contribution.id} cannot create commands`);
    }
    const command = await this.withContribution(selection, signal, (contribution, context) =>
      contribution.create!(immutableProjectSnapshot(project), context),
    );
    assertNewCommand(project, command);
    return cloneStoryValue(command);
  }

  private commandSchemaSelection(selector: number | string): ActiveSelection<"command"> | undefined {
    const schemas = this.orderedSelections("command");
    if (typeof selector === "number") {
      return schemas.find(({ contribution }) => contribution.opcodes?.includes(selector));
    }
    ...
    return schemas.find(
      ({ contribution, owner }) =>
        ...
        contribution.sourceNames?.some((name) => name.toLowerCase() === normalized),
    );
  }
  ...
  async runAi(
    ...
    const result = await this.withContribution(selection, signal, (provider, context) =>
  ...
  install(plugin: AltairPlugin, options: AltairPluginInstallOptions = {}): Promise<void> {
    if (!this.acceptingOperations) {
      return Promise.reject(new ReferenceError("Altair plugin host is disposing"));
    }
    ...
    if (this.installed.has(manifest.id) || this.scheduledInstalls.has(manifest.id)) {
      return Promise.reject(new Error(`Altair plugin already installed or installing: ${manifest.id}`));
    }
    this.scheduledInstalls.add(manifest.id);
    const operation = this.enqueue(() => this.installNow(plugin, manifest, installOptions));
    void operation.then(
  ...
  async sniffFormats(request: AltairFormatRequest): Promise<readonly AltairFormatSniffResult[]> {
    const normalized = normalizeFormatRequest(request);
    throwIfAborted(normalized.signal);
    const results = await Promise.all(
      this.orderedSelections("format").map(async (selection) => {
        const score = await this.withContribution(selection, normalized.signal, (contribution, context) =>
          contribution.sniff({ ...normalized, signal: context.signal }, context),
        );
      ...
      results
        ...
        .sort(
          (left, right) =>
            ...
            stableText(`${left.owner}:${left.id}`, `${right.owner}:${right.id}`),
        ),
    );
  }

  async importFormat(request: AltairFormatRequest, formatId?: string): Promise<AltairFormatImportResult> {
    const normalized = normalizeFormatRequest(request);
    const selection = formatId
      ? this.requireSelection("format", formatId)
      : await this.detectFormatSelection(normalized);
    const result = await this.withContribution(selection, normalized.signal, (contribution, context) =>
      contribution.import({ ...normalized, signal: context.signal }, context),
    );
    if (!result || typeof result.format !== "string" || !result.format.trim()) {
      throw new TypeError(`Altair format ${selection.owner}:${selection.contribution.id} returned no format id`);
    }
  ...
  async exportFormat(formatId: string, request: AltairFormatExportRequest): Promise<AltairFormatExportResult> {
    ...
    const result = await this.withContribution(selection, normalized.signal, (contribution, context) =>
      contribution.export({ ...normalized, signal: context.signal }, context),
    );
  ...
  async runEditorAction(actionId: string, request: AltairEditorActionRequest): Promise<StoryProject> {
    assertStoryProjectProtocol(request.project);
    const selection = this.requireSelection("editor-action", actionId);
    const signal = request.signal ?? neverAbortedSignal();
    const project = await this.withContribution(selection, signal, (contribution, context) =>
      contribution.run(
        {
          ...request,
  ...
  async runCompilerPasses(request: AltairCompilerPassRequest): Promise<AltairCompilerPipelineResult> {
    assertStoryProjectProtocol(request.project);
    const scene = request.project.scenes.find(({ id }) => id === request.sceneId);
    if (!scene) {
      throw new RangeError(`Unknown Altair scene: ${request.sceneId}`);
    }
    if (!Number.isSafeInteger(request.commandIndex) || request.commandIndex < 0) {
      throw new RangeError("Altair compiler command index must be non-negative");
    }
    assertCommandInProject(request.project, request.sceneId, request.commandIndex, request.command);
    if (request.output !== undefined && request.output !== null && !isJsonObject(request.output)) {
      throw new TypeError("Altair compiler output must be a JSON object or null");
    }
    const signal = request.signal ?? neverAbortedSignal();
    const projectSnapshot = immutableProjectSnapshot(request.project);
    let command = cloneStoryValue(request.command);
    let output = request.output === undefined ? undefined : cloneStoryValue(request.output);
    ...
    const passes = this.orderedSelections("compiler").sort(
      (left, right) =>
        ...
        right.priority - left.priority ||
        stableText(`${left.owner}:${left.contribution.id}`, `${right.owner}:${right.contribution.id}`),
    );
    for (const selection of passes) {
      const result = await this.withContribution(selection, signal, (contribution, context) =>
        contribution.apply(
          {
            ...
            ...(output === undefined ? {} : { output: cloneStoryValue(output) }),
            signal: context.signal,
          },
          context,
        ),
      ...
      if (result.command !== undefined) {
        ...
        assertCommandInProject(request.project, request.sceneId, request.commandIndex, command);
      }
      if (Object.hasOwn(result, "output")) {
        if (result.output !== null && result.output !== undefined && !isJsonObject(result.output)) {
          throw new TypeError(
            `Altair compiler ${selection.owner}:${selection.contribution.id} returned a non-object output`,
          );
        }
        output = result.output === undefined ? undefined : cloneStoryValue(result.output);
      }
  ...
  async evaluateDiagnostics(
    ...
    const projectSnapshot = immutableProjectSnapshot(project);
    const diagnostics: StoryDiagnostic[] = [];
    for (const selection of this.orderedSelections("validator")) {
      const values = await this.withContribution(selection, signal, (contribution, context) =>
        contribution.validate(projectSnapshot, context),
      );
  ...
  async buildFlow<T extends AltairFlowGraph = AltairFlowGraph>(
    ...
    const projectSnapshot = immutableProjectSnapshot(project);
    const graph = await this.withContribution(selection, signal, (contribution, context) =>
      contribution.build(projectSnapshot, context),
    );
    ...
    return cloneStoryValue(graph) as unknown as T;
  }

  async resolveAsset(request: AltairAssetRequest, providerId?: string): Promise<AltairResolvedAsset | undefined> {
    assertStoryProjectProtocol(request.project);
    ...
    const normalizedRequest = Object.freeze({
      project: immutableProjectSnapshot(request.project),
      reference: request.reference.trim(),
      ...(request.kind === undefined ? {} : { kind: request.kind.trim() }),
      signal,
    });
    const candidates = providerId ? [this.requireSelection("asset", providerId)] : this.orderedSelections("asset");
    ...
    for (const selection of candidates) {
      const value = selection.contribution.supports
        ? await this.withContribution(selection, signal, (contribution, context) =>
            contribution.supports!({ ...normalizedRequest, signal: context.signal }, context),
          )
        : 1;
      const score = typeof value === "boolean" ? (value ? 1 : 0) : Number(value);
    ...
    for (const { selection } of supported) {
      const resolved = await this.withContribution(selection, signal, (contribution, context) =>
        contribution.resolve({ ...normalizedRequest, signal: context.signal }, context),
      );
  ...
  async previewDocument(
    ...
    project: StoryProject,
    signal: AbortSignal = neverAbortedSignal(),
  ): Promise<AltairDocumentPreview | undefined> {
    assertStoryProjectProtocol(project);
    const selection = this.requireSelection("document-editor", editorId);
    if (!selection.contribution.preview) return undefined;
    const result = await this.withContribution(selection, signal, (editor, context) =>
      editor.preview!({ path: document.path, text: document.text }, immutableProjectSnapshot(project), context),
    );
    if (!result?.scene) throw new TypeError(`Document editor ${editorId} returned no preview scene`);
    assertStoryProjectProtocol({ ...project, scenes: [...project.scenes, result.scene] });
    if (result.commandId !== undefined && !result.scene.commands.some((command) => command.id === result.commandId))
      throw new TypeError(`Document editor ${editorId} returned an unknown preview command`);
    return cloneStoryValue(result);
  }
      ...
      await settleWithAbort(
        Promise.resolve(
          selection.contribution.mount(host, {
            ...
            service: (key) => this.serviceForOwner(selection.owner, key),
            ...(options.editor ? { editor: options.editor } : {}),
            ...(options.project === undefined ? {} : { project: cloneStoryValue(options.project) }),
            ...(options.selection === undefined ? {} : { selection: cloneStoryValue(options.selection) }),
          }),
        ).then((resource) => lease.capture(resource, linked.signal)),
        linked.signal,
      );
      throwIfAborted(linked.signal);
      return this.ownRuntimeResource(selection.owner, lease, local, linked.dispose);
    } catch (error) {
      local.abort(error);
      linked.dispose();
      await lease.dispose();
  ...
  async createPreview(providerId: string, request: AltairPreviewRequest = {}): Promise<AltairPreviewSession> {
    ...
    try {
      ...
      const session = await settleWithAbort(
        Promise.resolve(
          selection.contribution.create(
            {
              ...request,
              ...(request.project === undefined ? {} : { project: cloneStoryValue(request.project) }),
              signal: linked.signal,
            },
            {
              signal: linked.signal,
      ...
      throwIfAborted(linked.signal);
      const owned = this.ownRuntimeResource(selection.owner, lease, local, linked.dispose);
      return Object.freeze({
        ...(session.capabilities === undefined ? {} : { capabilities: Object.freeze([...session.capabilities]) }),
        request: async (command: JsonObject, options: { readonly signal?: AbortSignal } = {}): Promise<JsonValue> => {
          ...
          const requestSignal = linkAbortSignals([linked.signal, options.signal]);
          try {
  ...
  dispose(): Promise<void> {
    ...
    this.hostController.abort(new DOMException("Altair plugin host disposed", "AbortError"));
    this.disposal = this.operationTail.then(async () => {
      const errors: unknown[] = [];
      while (this.installed.size) {
        const removable = [...this.installed.keys()].find(
          (pluginId) =>
            ![...this.installed.values()].some(({ manifest }) => Object.hasOwn(manifest.dependencies ?? {}, pluginId)),
        );
        if (!removable) {
          errors.push(new Error("Altair plugin dependency graph contains a cycle"));
          break;
        }
        try {
      ...
      if (errors.length) {
        throw new AggregateError(errors, "Failed to dispose the Altair plugin host");
      }
    });
  ...
  private async installNow(
    ...
    transaction.cleanups.push(forwardAbort(this.hostController.signal, transaction.controller));
    ...
    try {
        ...
        manifest.apiVersion === 1
          ...
          : await (plugin as AltairPluginV2).setup({
              ...
              contribute: (kind, contribution, options) => {
                this.requireCapability(manifest, CAPABILITY_BY_CONTRIBUTION[kind]);
                return this.stageContribution(transaction, kind, contribution, options);
              },
              provide: (key, value) => {
              ...
              defer: (cleanup) => this.stageCleanup(transaction, cleanup),
            });
      ...
      if (cleanupErrors.length) {
        throw new AggregateError([error, ...cleanupErrors], `Failed to install ${manifest.id}`);
      }
      throw error;
    }
  }

  private createLegacyContext(transaction: InstallTransaction): AltairPluginContext {
    return {
      ...
      hasPermission: (permission) => transaction.installOptions.permissionSet.has(permission),
      registerFormat: (codec) => {
        this.requireCapability(transaction.manifest, "format");
        const snapshot = snapshotLegacyFormat(codec);
        const removeLegacy = this.stageLegacy(transaction, this.legacyFormats, snapshot.id, snapshot, "format");
      ...
      registerAiProvider: (provider) => {
        ...
        const snapshot = snapshotContribution("ai", provider) as AltairAiProvider;
        ...
        const removeContribution = this.stageContribution(transaction, "ai", snapshot, undefined);
        return onceCleanup(async () => {
          await removeContribution();
          await removeLegacy();
        });
  ...
  private async removeNow(pluginId: string): Promise<void> {
    ...
    if (dependants.length) {
      throw new Error(`${pluginId} is required by ${dependants.join(", ")}`);
    }
    this.installed.delete(pluginId);
    installed.controller.abort(new DOMException(`Altair plugin removed: ${pluginId}`, "AbortError"));
    const errors: unknown[] = [];
    try {
  ...
  private requireCapability(manifest: AltairPluginManifest, capability: AltairPluginCapability): void {
    if (!manifest.capabilities?.includes(capability)) {
      throw new Error(`${manifest.id} must declare the '${capability}' capability before registering it`);
    }
  }
  ...
  private stageContribution<K extends keyof AltairContributionMap>(
    ...
    const registryKey = contributionRegistryKey(transaction.manifest.id, value.id);
    const registry =
    ...
    if (
      ...
      throw new Error(`Altair ${kind} contribution already registered: ${value.id}`);
    }
    const normalized = normalizeContributionOptions(transaction.manifest.id, kind, options);
  ...
  private stageService<T>(transaction: InstallTransaction, key: AltairServiceKey<T>, value: T): () => void {
    requireServiceId(key);
    if (this.serviceRegistry.has(key.id) || transaction.stagedServices.has(key.id)) {
  ...
  private stageLegacy<T>(
    ...
    const registration = new StagedMapRegistration(id, { owner: transaction.manifest.id, value }, map);
    transaction.registrations.push(registration);
    const remove = onceCleanup(() => registration.remove());
    transaction.cleanups.push(remove);
    return remove;
  ...
  private stageCleanup(transaction: InstallTransaction, cleanup: Cleanup): () => void {
  ...
  private legacySnapshot<T>(registry: Map<string, RegisteredLegacy<T>>): Map<string, T> {
    return new Map(
      [...registry.entries()]
  ...
  private orderedSelections<K extends keyof AltairContributionMap>(kind: K): ActiveSelection<K>[] {
    return this.contributionSelections(kind)
      .filter(
        (selection): selection is AltairContributionSelection<AltairContributionMap[K]> & { readonly active: true } =>
          selection.active,
      )
      .sort(
        (left, right) =>
  ...
  private requireSelection<K extends keyof AltairContributionMap>(kind: K, selector: string): ActiveSelection<K> {
    if (typeof selector !== "string" || !selector.trim()) {
      throw new TypeError(`Altair ${kind} selector must not be empty`);
    }
    const selected = this.orderedSelections(kind).find(
      ({ owner, contribution }) => contribution.id === selector || `${owner}:${contribution.id}` === selector,
    );
    if (!selected) {
  ...
  private async withContribution<K extends keyof AltairContributionMap, T>(
    selection: ActiveSelection<K>,
    callerSignal: AbortSignal,
    run: (contribution: AltairContributionMap[K], context: AltairOperationContext) => T | Promise<T>,
  ): Promise<T> {
    const linked = linkAbortSignals([this.hostController.signal, this.ownerSignal(selection.owner), callerSignal]);
    try {
      throwIfAborted(linked.signal);
   ...
   */
  private serviceForOwner<T>(owner: string, key: AltairServiceKey<T>): T | undefined {
    const manifest = this.installed.get(owner)?.manifest;
    return manifest ? this.serviceForManifest(manifest, key) : undefined;
  }

  private serviceForManifest<T>(manifest: AltairPluginManifest, key: AltairServiceKey<T>): T | undefined {
    requireServiceId(key);
    const service = this.serviceRegistry.get(key.id);
    if (
      !service ||
      !this.installed.has(service.owner) ||
      (service.owner !== manifest.id && !Object.hasOwn(manifest.dependencies ?? {}, service.owner))
    ) {
      return undefined;
    }
  ...
  private ownerSignal(owner: string): AbortSignal {
    ...
    if (!installed) {
      ...
      controller.abort(new DOMException(`Altair plugin is not active: ${owner}`, "AbortError"));
  ...
  private ownRuntimeResource(
    ...
    const installed = this.installed.get(owner);
    if (!installed) {
      local.abort(new DOMException(`Altair plugin is not active: ${owner}`, "AbortError"));
      unlink();
    ...
    const dispose = onceCleanup(async () => {
      local.abort(new DOMException("Altair contributed resource disposed", "AbortError"));
      unlink();
      removeOwnedCleanup();
...
type ActiveSelection<K extends keyof AltairContributionMap> = AltairContributionSelection<AltairContributionMap[K]> & {
...
class StagedContributionRegistration implements StagedRegistration {
  ...
  constructor(
    ...
    private readonly registry: Map<string, RegisteredContribution<AltairContribution>>,
  ...
  commit(): void {
    ...
    if (this.registry.has(this.registryKey)) {
      throw new Error(`Altair ${this.kind} contribution collided during commit: ${this.id}`);
    }
  ...
  remove(): void {
    if (!this.active) return;
    this.active = false;
    if (this.committed && this.registry.get(this.registryKey) === this.value) {
...
const INSTALL_PERMISSION = /^[a-z0-9][a-z0-9._*-]*(?::[a-z0-9._*-]+)*$/u;
const DANGEROUS_CONFIGURATION_KEYS = new Set(["__proto__", "constructor", "prototype"]);
...
const snapshotInstallJson = (
  ...
  if (depth > INSTALL_CONFIGURATION_MAX_DEPTH) {
    throw new RangeError("Altair plugin configuration is too deeply nested");
  }
  ...
  try {
    if (Array.isArray(value)) {
      if (Object.keys(value).length !== value.length) {
        throw new TypeError(`${path} must not contain sparse array values`);
      }
      return Object.freeze(
        value.map((item, index) => snapshotInstallJson(item, `${path}[${index}]`, seen, state, depth + 1)),
    ...
    for (const key of Object.keys(value)) {
      ...
      output[key] = snapshotInstallJson(descriptor.value, `${path}.${key}`, seen, state, depth + 1);
    }
...
const normalizeInstallOptions = (options: AltairPluginInstallOptions): NormalizedInstallOptions => {
  ...
  if (options.permissions !== undefined && !Array.isArray(options.permissions)) {
    throw new TypeError("Altair plugin permissions must be an array");
  }
  const permissions = Object.freeze(
    [...(options.permissions ?? [])].map((permission) => {
      if (typeof permission !== "string" || permission.length > 128 || !INSTALL_PERMISSION.test(permission)) {
        throw new TypeError(`Invalid Altair plugin permission: ${String(permission)}`);
      }
      return permission;
    }),
  );
  ...
  if (new Set(permissions).size !== permissions.length) {
    throw new TypeError("Altair plugin permissions contain a duplicate");
  }
  return Object.freeze({
...
const snapshotManifest = (candidate: AltairPluginManifest): AltairPluginManifest => {
  ...
  if (apiVersion !== 1 && apiVersion !== ALTAIR_PLUGIN_API_VERSION) {
    throw new Error(`Unsupported Altair plugin API: ${String(apiVersion)}`);
  }
  ...
  if (capabilities.some((capability) => !PLUGIN_CAPABILITIES.has(capability))) {
    throw new TypeError(`${candidate.id} declares an unknown capability`);
  }
  if (new Set(capabilities).size !== capabilities.length) {
    throw new TypeError(`${candidate.id} declares a capability more than once`);
  }
  const dependencies: Record<string, string> = {};
  for (const [dependency, range] of Object.entries(candidate.dependencies ?? {}).sort(([left], [right]) =>
    stableText(left, right),
  )) {
    if (!PLUGIN_ID.test(dependency) || dependency === candidate.id) {
      throw new TypeError(`${candidate.id} has an invalid dependency ${dependency}`);
    }
    if (typeof range !== "string" || !validRange(range)) {
      throw new TypeError(`${candidate.id} has an invalid dependency range for ${dependency}`);
    }
  ...
  return Object.freeze({
    ...
    ...(Object.keys(dependencies).length ? { dependencies: Object.freeze(dependencies) } : {}),
    ...(capabilities.length ? { capabilities: Object.freeze([...capabilities]) } : {}),
  }) as AltairPluginManifest;
};

const snapshotLegacyFormat = (codec: AltairFormatCodec): AltairFormatCodec => {
...
const snapshotLegacyDiagnosticRule = (rule: AltairDiagnosticRule): AltairDiagnosticRule => {
...
const snapshotContribution = <K extends keyof AltairContributionMap>(
  ...
  switch (kind) {
    case "format": {
      ...
      requireStringArray(value.extensions, `Altair format ${value.id} extensions`);
      if (value.mediaTypes !== undefined) {
        requireStringArray(value.mediaTypes, `Altair format ${value.id} mediaTypes`);
      }
      requireMethods(value, kind, ["sniff", "import", "export"]);
      return Object.freeze({
        ...base,
        extensions: Object.freeze([...value.extensions]),
        ...(value.mediaTypes === undefined ? {} : { mediaTypes: Object.freeze([...value.mediaTypes]) }),
    ...
    case "command": {
      ...
      if (value.document !== undefined) {
        ...
        if (
          !document.type ||
          !nonEmpty(document.type.plugin) ||
          !nonEmpty(document.type.name) ||
          !Number.isSafeInteger(document.schemaVersion) ||
          document.schemaVersion < 1 ||
          typeof document.lower !== "function" ||
          (document.migrate !== undefined && typeof document.migrate !== "function")
        )
          throw new TypeError(`Invalid document command: ${value.id}`);
      }

      if (
        value.opcodes !== undefined &&
        (!Array.isArray(value.opcodes) ||
          value.opcodes.some((opcode) => !Number.isSafeInteger(opcode) || opcode < 0) ||
          new Set(value.opcodes).size !== value.opcodes.length)
      ) {
        throw new TypeError(`Altair command schema ${value.id} has an invalid opcode`);
      }
      if (value.sourceNames !== undefined) {
        requireStringArray(value.sourceNames, `Altair command schema ${value.id} sourceNames`);
      }
      if (value.fields !== undefined) {
        requireCommandFields(value.fields, value.id);
      }
      if (value.category !== undefined && !nonEmpty(value.category)) {
        throw new TypeError(`Altair command schema ${value.id} has an invalid category`);
      }
      if (value.metadata !== undefined && (!isJsonObject(value.metadata) || !isJsonValue(value.metadata))) {
        throw new TypeError(`Altair command schema ${value.id} has invalid metadata`);
      }
      if (value.create !== undefined && typeof value.create !== "function") {
        throw new TypeError(`Altair command schema ${value.id} has an invalid create method`);
      }
      return Object.freeze({
        ...base,
        ...(value.document === undefined
          ...
          : {
              document: Object.freeze({
                ...value.document,
                type: Object.freeze({ ...value.document.type }),
              }),
            }),
        ...(value.opcodes === undefined ? {} : { opcodes: Object.freeze([...value.opcodes]) }),
        ...(value.sourceNames === undefined ? {} : { sourceNames: Object.freeze([...value.sourceNames]) }),
        ...(value.metadata === undefined ? {} : { metadata: cloneStoryValue(value.metadata) }),
              ...
              fields: Object.freeze(
                value.fields.map((field) =>
                  Object.freeze({
                    ...
                    ...(field.metadata === undefined ? {} : { metadata: cloneStoryValue(field.metadata) }),
                  }),
                ),
              ),
            }),
    ...
    case "editor-action":
      if (!nonEmpty((candidate as AltairContributionMap["editor-action"]).label)) {
        throw new TypeError(`Altair editor action ${candidate.id} has no label`);
      }
      if (
        (candidate as AltairContributionMap["editor-action"]).location !== undefined &&
        !nonEmpty((candidate as AltairContributionMap["editor-action"]).location)
      ) {
        throw new TypeError(`Altair editor action ${candidate.id} has an invalid location`);
      }
    ...
    case "compiler": {
      ...
      if (value.order !== undefined && !Number.isSafeInteger(value.order)) {
        throw new TypeError(`Altair compiler pass ${value.id} has an invalid order`);
      }
    ...
    case "asset": {
      ...
      if (value.supports !== undefined && typeof value.supports !== "function") {
        throw new TypeError(`Altair asset provider ${value.id} has an invalid supports method`);
      }
    ...
    case "resource-browser": {
      ...
      if (!nonEmpty(value.name)) {
        throw new TypeError(`Altair resource browser ${value.id} has no name`);
      }
      if (!Array.isArray(value.roots) || value.roots.length === 0) {
        throw new TypeError(`Altair resource browser ${value.id} has no roots`);
      }
      ...
      const roots = value.roots.map((root: ResourceBrowserDirectory) => {
        if (
          ...
          throw new TypeError(`Altair resource browser ${value.id} has an invalid root`);
        }
        const rootId = root.id.trim();
        if (rootIds.has(rootId)) {
          throw new TypeError(`Altair resource browser ${value.id} duplicates root ${rootId}`);
        }
        rootIds.add(rootId);
        return Object.freeze({
          ...root,
      ...
      requireMethods(value, kind, ["preferredPath", "list", "open"]);
      for (const method of ["refresh", "dispose"] as const) {
        if (value[method] !== undefined && typeof value[method] !== "function") {
          throw new TypeError(`Altair resource browser ${value.id} has an invalid ${method} method`);
        }
      }
      return Object.freeze({
        ...base,
    ...
    case "document-editor": {
      const value = candidate as AltairContributionMap["document-editor"];
      if (!nonEmpty(value.label) || (value.panelId !== undefined && !nonEmpty(value.panelId)))
        throw new TypeError(`Altair document editor ${value.id} has an invalid label or panel`);
      if (value.affectsPreview !== undefined && typeof value.affectsPreview !== "boolean")
        throw new TypeError(`Altair document editor ${value.id} has an invalid preview flag`);
      requireMethods(candidate, kind, ["matches"]);
      for (const key of ["create", "validate", "preview"] as const)
        if (value[key] !== undefined && typeof value[key] !== "function")
          throw new TypeError(`Invalid document editor ${key}`);
      break;
    }
    case "property-editor": {
      const value = candidate as AltairContributionMap["property-editor"];
      if (!nonEmpty(value.panelId)) throw new TypeError(`Altair property editor ${value.id} requires a panel`);
      requireMethods(candidate, kind, ["matches"]);
      break;
    }
    case "panel": {
...
const legacyFormatContribution = (codec: AltairFormatCodec): AltairFormatContribution => ({
  id: codec.id,
  extensions: codec.extensions,
  sniff: ({ files, entryPath }) => codec.sniff(entryFile(files, entryPath).bytes),
  ...
  async export({ project, entryPath }) {
    return {
      artifacts: [
        {
          path: entryPath ?? `export${codec.extensions[0]?.startsWith(".") ? codec.extensions[0] : ".bin"}`,
...
const legacyValidatorContribution = (rule: AltairDiagnosticRule): AltairValidatorContribution => ({
  id: rule.id,
...
const normalizeContributionOptions = (
  ...
  options: AltairContributionOptions | undefined,
): Pick<RegisteredContribution<AltairContribution>, "priority" | "overrides" | "singletonPort"> => {
  ...
  if (options?.singletonPort !== undefined && !nonEmpty(options.singletonPort)) {
    throw new TypeError(`Plugin ${owner} supplied an invalid ${kind} singleton port`);
  }
  return {
    priority,
    overrides: Object.freeze([...new Set(rawOverrides.map((selector) => selector.trim()))].sort(stableText)),
...
const selectContributionWinners = (
...
): ReadonlyMap<RegisteredContribution<AltairContribution>, RegisteredContribution<AltairContribution>> => {
  const parent = new Map(records.map((record) => [record, record] as const));
  const find = (record: RegisteredContribution<AltairContribution>): RegisteredContribution<AltairContribution> => {
  ...
  const union = (
    ...
    const b = find(right);
    if (a === b) return;
    const first = stableText(qualifiedContributionId(a), qualifiedContributionId(b)) <= 0 ? a : b;
    parent.set(a === first ? b : a, first);
  };
  const singletonOwners = new Map<string, RegisteredContribution<AltairContribution>>();
  ...
  for (const record of records) {
    for (const selector of record.overrides) {
      for (const target of records) {
        if (record !== target && (selector === target.value.id || selector === qualifiedContributionId(target))) {
          union(record, target);
        }
      }
    }
  }
  const groups = new Map<RegisteredContribution<AltairContribution>, RegisteredContribution<AltairContribution>[]>();
  for (const record of records) {
  ...
  const winners = new Map<RegisteredContribution<AltairContribution>, RegisteredContribution<AltairContribution>>();
...
const preferredContribution = (
  ...
  return stableText(qualifiedContributionId(left), qualifiedContributionId(right)) <= 0 ? left : right;
};
...
const qualifiedContributionId = (contribution: RegisteredContribution<AltairContribution>): string =>
  `${contribution.owner}:${contribution.value.id}`;

const contributionRegistryKey = (owner: string, id: string): string => `${owner}\0${id}`;
...
const normalizeFormatRequest = (
  ...
  if (entryPath && !files.some(({ path }) => path === entryPath)) {
    throw new RangeError(`Altair format entry is not present in files: ${entryPath}`);
  }
  return Object.freeze({
    files,
    ...(entryPath ? { entryPath } : {}),
    ...(request.options === undefined ? {} : { options: cloneStoryValue(request.options) }),
...
const normalizeFormatExportRequest = (
  ...
  Object.freeze({
    project: immutableProjectSnapshot(request.project),
    ...(nonEmpty(request.entryPath) ? { entryPath: request.entryPath!.trim() } : {}),
    ...(request.options === undefined ? {} : { options: cloneStoryValue(request.options) }),
    signal: request.signal ?? neverAbortedSignal(),
  });
...
const normalizeSourceFiles = (
  ...
  return Object.freeze(
    files.map((file) => {
      ...
      if (file.mediaType !== undefined && !nonEmpty(file.mediaType)) {
        throw new TypeError(`Altair source file ${path} has an invalid media type`);
      }
      return Object.freeze({
        path,
        bytes: file.bytes,
        ...(file.mediaType === undefined ? {} : { mediaType: file.mediaType.trim() }),
      });
    }),
  );
};
...
const entryFile = (
  ...
  const file = entryPath ? files.find(({ path }) => path === entryPath) : files[0];
  if (!file) throw new RangeError(`Altair format entry does not exist`);
  return file;
};

const normalizeArtifacts = (artifacts: readonly AltairFormatArtifact[]): readonly AltairFormatArtifact[] => {
...
const normalizeResolvedAsset = (
  ...
  if (asset.source === undefined && !(asset.bytes instanceof Uint8Array)) {
    throw new TypeError(`Altair asset ${asset.id} must provide source or bytes`);
  }
  ...
  if (asset.metadata !== undefined && (!isJsonObject(asset.metadata) || !isJsonValue(asset.metadata))) {
  ...
  return Object.freeze({
    ...
    ...(asset.mediaType === undefined ? {} : { mediaType: asset.mediaType.trim() }),
    ...(asset.metadata === undefined ? {} : { metadata: cloneStoryValue(asset.metadata) }),
  });
};
...
const assertCommandInProject = (
  ...
  const sceneIndex = project.scenes.findIndex(({ id }) => id === sceneId);
  const scene = project.scenes[sceneIndex];
  if (!scene || commandIndex >= scene.commands.length) {
    throw new RangeError(`Altair compiler command index ${commandIndex} does not exist in ${sceneId}`);
  }
  const commands = [...scene.commands];
  commands[commandIndex] = cloneStoryValue(command);
  const candidate: StoryProject = {
    ...project,
    scenes: project.scenes.map((item, index) => (index === sceneIndex ? { ...scene, commands } : item)),
  };
  assertStoryProjectProtocol(candidate);
};

const assertNewCommand = (project: StoryProject, command: StoryProjectCommand): void => {
  const entrySceneIndex = project.scenes.findIndex(({ id }) => id === project.entrySceneId);
  const sceneIndex = entrySceneIndex >= 0 ? entrySceneIndex : 0;
  const scene = project.scenes[sceneIndex];
  if (!scene) throw new Error("Altair project has no scene");
  const candidate: StoryProject = {
    ...project,
    scenes: project.scenes.map((item, index) =>
      index === sceneIndex ? { ...scene, commands: [...scene.commands, cloneStoryValue(command)] } : item,
    ),
  };
  assertStoryProjectProtocol(candidate);
};

const cloneAdaptationRequest = (request: AltairAdaptationRequest): AltairAdaptationRequest => {
  ...
  return Object.freeze({
    ...
    ...(request.existingProject === undefined ? {} : { existingProject: cloneStoryValue(request.existingProject) }),
  });
};

const normalizeAdaptationResult = (providerId: string, result: AltairAdaptationResult): AltairAdaptationResult => {
  if (!result || typeof result !== "object") {
  ...
  if (
    ...
    (provenance.sourceHash !== undefined && typeof provenance.sourceHash !== "string")
  ) {
    throw new TypeError(`AI provider '${providerId}' returned invalid provenance`);
  }
  return Object.freeze({
    project: cloneStoryValue(result.project),
    diagnostics: Object.freeze(cloneStoryValue([...result.diagnostics])),
...
const requireDiagnostics = (diagnostics: readonly StoryDiagnostic[], source: string): void => {
...
const requireMethods = (value: object, kind: string, methods: readonly string[]): void => {
  for (const method of methods) {
    if (typeof (value as unknown as Record<string, unknown>)[method] !== "function") {
      throw new TypeError(`Altair ${kind} contribution has no ${method} method`);
    }
  }
};

const requireStringArray = (value: readonly string[], label: string): void => {
  if (!Array.isArray(value) || value.some((entry) => !nonEmpty(entry)) || new Set(value).size !== value.length) {
...
const requireCommandFields = (
  ...
  if (!Array.isArray(fields)) {
    throw new TypeError(`Altair command schema ${schemaId} fields must be an array`);
  }
  ...
  for (const field of fields) {
    if (
      ...
      throw new TypeError(`Altair command schema ${schemaId} has an invalid field`);
    }
    const key = field.key.trim();
    if (keys.has(key)) {
      throw new TypeError(`Altair command schema ${schemaId} repeats field ${key}`);
    }
    keys.add(key);
    if (field.resourceKind !== undefined && !nonEmpty(field.resourceKind)) {
      throw new TypeError(`Altair command schema ${schemaId} field ${key} has an invalid resource kind`);
    }
    if (field.metadata !== undefined && (!isJsonObject(field.metadata) || !isJsonValue(field.metadata))) {
      throw new TypeError(`Altair command schema ${schemaId} field ${key} has invalid metadata`);
    }
    if (field.options === undefined) continue;
    if (!Array.isArray(field.options)) {
      throw new TypeError(`Altair command schema ${schemaId} field ${key} options must be an array`);
    }
    for (const option of field.options) {
      if (!option || typeof option !== "object" || !nonEmpty(option.label) || !isJsonValue(option.value)) {
        throw new TypeError(`Altair command schema ${schemaId} field ${key} has an invalid option`);
      }
    }
  }
};
...
const requireActivation = (value: unknown, pluginId: string): AltairPluginActivation => {
  if (!value || typeof value !== "object") {
    throw new TypeError(`Altair plugin ${pluginId} returned an invalid activation`);
  }
  for (const method of ["start", "stop", "dispose"] as const) {
    if (method in value && typeof (value as Record<string, unknown>)[method] !== "function") {
      throw new TypeError(`Altair plugin ${pluginId} activation has an invalid ${method} method`);
    }
  }
...
const disposableLease = () => {
  let closed = false,
    release: Cleanup | undefined;
  return {
    async capture<T extends AltairDisposable>(resource: T, signal: AbortSignal): Promise<T> {
      release = onceCleanup(toCleanup(resource));
...
const runCleanups = async (cleanups: Cleanup[], errors: unknown[]): Promise<void> => {
...
const abortReason = (signal: AbortSignal): unknown =>
  signal.reason ?? new DOMException("The Altair operation was aborted", "AbortError");

const settleWithAbort = <T>(operation: T | PromiseLike<T>, signal: AbortSignal): Promise<T> => {
...
const forwardAbort = (source: AbortSignal, target: AbortController): Cleanup => {
  if (source.aborted) {
...
const freezeObjectTree = <T>(value: T): T => {
...
const immutableProjectSnapshot = (project: StoryProject): StoryProject => freezeObjectTree(cloneStoryValue(project));
